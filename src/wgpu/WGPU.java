...
public class WGPU {
	...
	public static void wgpuAdapterRequestDevice(final long adapter, @Nullable final DeviceDescriptor descriptor, final AdapterRequestDeviceCallback callback, final long userdata) {
		try(var stack = Stack.get()) {
			var _adapter = adapter;
			var _descriptor = stack.alloc(descriptor);
			var _callback = Callback.createStub(callback, callback.handle, callback.desc);
	...
	public static void wgpuQueueSubmit(final long queue, final GPUCommandBuffer[] commands) {
		try(var stack = Stack.get()) {
			var _queue = queue;
	...
	public static void wgpuRenderPassEncoderExecuteBundles(final long renderPassEncoder, final GPURenderBundle[] bundles) {
		try(var stack = Stack.get()) {
			var _renderPassEncoder = renderPassEncoder;
	...
	public static void wgpuSurfaceGetCapabilities(final long surface, final long adapter, SurfaceCapabilities capabilities) {
		try(var stack = Stack.get()) {
			...
			((CStruct)capabilities).load(_capabilities);
	...
	public static void wgpuSurfaceGetCurrentTexture(final long surface, SurfaceTexture surfaceTexture) {
		try(var stack = Stack.get()) {
			...
			((CStruct)surfaceTexture).load(_surfaceTexture);
		} catch (Throwable e) {
			throw new RuntimeException(e);
		}
	}
	...
	public static long wgpuInstanceEnumerateAdapters(final long instance, @Nullable final InstanceEnumerateAdapterOptions options, GPUAdapter[] adapters) {
		try(var stack = Stack.get()) {
			var _instance = instance;
			var _options = stack.alloc(options);
			...
			if(adapters != null) {
				for(int i=0; i<adapters.length; i++) {
					adapters[i] = new GPUAdapter(get_long(_adapters + i*8));
				}
			}
			return ret;
		} catch (Throwable e) {
	...
	public static long wgpuQueueSubmitForIndex(final long queue, final GPUCommandBuffer[] commands) {
		try(var stack = Stack.get()) {
			var _queue = queue;
			var _commandCount = (long) (commands == null ? 0 : commands.length);
			var _commands = stack.alloc(commands);
