...
public class ShaderModuleWGSLDescriptor extends ChainedStruct {
	...
	protected long store(Stack stack, long address) {
		...
		put_value(address + 8, (int) SType.ShaderModuleWGSLDescriptor);
		// padding 4
		put_value(address+16, stack.alloc(code));
		return address;
	}

	protected ShaderModuleWGSLDescriptor load(long address) {
		var _next = get_long(address + 0);
		// unit32_t sType
		// padding 4
		code = get_string(get_long(address+16));
